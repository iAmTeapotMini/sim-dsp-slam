...
Такая гибридная структура существенно уменьшает количество частиц, необходимых для схождения алгоритма, повышает вычислительную стабильность и предотвращает быстрое вырождение весов.

---

## Валидация алгоритма
Для проверки качества работы реализованного фильтра RBPF в среде симуляции генерируются синтетические наблюдения $y_k$ на основе заданных истинных траекторий параметров диполя (Ground Truth), после чего оценивается среднеквадратичная ошибка (RMSE) восстановления состояния во времени.
