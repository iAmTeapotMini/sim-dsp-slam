...
def run_graph_slam(g, numIterations):
    ...
    for i in range(numIterations):
        ...
        print(f"Iteration {i + 1}, Global error: {error:.6f}")
...
def compute_global_error(g):
    ...
    for edge in g.edges:
        ...
        if edge.Type == 'P':
            ...
            e, _, _ = linearize_pose_pose_constraint(x1, x2, z12)
            Fx += e.T @ Omega @ e
        ...
        # pose-landmark constraint
            ...
            # (TODO) compute the error due to this edge
            e, _, _ = linearize_pose_landmark_constraint(x, l, z)
            Fx += e.T @ Omega @ e
...
def linearize_pose_pose_constraint(x1, x2, z):
    ...
    e = t2v(np.linalg.inv(Z12) @ (np.linalg.inv(X1) @ X2))

    theta_i = x1[2]
    theta_ij = z[2]

    Ri = np.array([[np.cos(theta_i), -np.sin(theta_i)],
                   [np.sin(theta_i), np.cos(theta_i)]])

    Rij = np.array([[np.cos(theta_ij), -np.sin(theta_ij)],
                    [np.sin(theta_ij), np.cos(theta_ij)]])

    obtain = np.array([[-np.sin(theta_i), np.cos(theta_i)],
                       [-np.cos(theta_i), -np.sin(theta_i)]])

    ti, tj = x1[0:2], x2[0:2]
    A = np.zeros((3, 3))
    A[0:2, 0:2] = -Rij.T @ Ri.T
    A[0:2, 2] = Rij.T @ obtain @ (tj - ti)
    A[2, 2] = -1

    B = np.zeros((3, 3))
    B[0:2, 0:2] = Rij.T @ Ri.T
    B[2, 2] = 1
...
def linearize_pose_landmark_constraint(x, l, z):
    ...
    obtain = np.array([[-np.sin(theta_i), np.cos(theta_i)],
                       [-np.cos(theta_i), -np.sin(theta_i)]])
    A = np.hstack([-Ri.T, np.dot(obtain, (l - ti).reshape(2,1))])
    B = Ri.T
